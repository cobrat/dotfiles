...
if status is-interactive
    ...
    if command -q fzf
        ...
        if command -q eza
            ...
            set -gx FZF_ALT_C_OPTS "--preview 'find {} -maxdepth 3 -print | head -200'"
        end
    ...
    if command -q eza
        ...
        function tree
            eza --tree --level=3 --all --ignore-glob='.git' --color=always $argv
        end

        function dtree
            eza --tree --level=3 --all --only-dirs --ignore-glob='.git' --color=always $argv
        end
    end
